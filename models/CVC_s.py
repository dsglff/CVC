...
class Encoder_PCA(nn.Module):
    def __init__(self, input_dim, input_time_dim, word_embedding, hidden_dim=768, num_heads=12, num_encoder_layers=1):
        super(Encoder_PCA, self).__init__()
        self.linear = nn.Linear(input_dim, hidden_dim)
        self.lineartime = nn.Linear(input_time_dim, hidden_dim)
...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        self.pred_len_add = ((self.pred_len + 3) // 4) * 4
        ...
        self.prompt_length = self.patch_num
        mode = 'symmetric' 
        ...
        self.prompt_pool = Prompt(length=1, embed_dim=768, embedding_key='mean', prompt_init='uniform', prompt_pool=False, 
                 prompt_key=True, pool_size=configs.pool_size, top_k=self.prompt_length, batchwise_prompt=False, prompt_key_init=configs.prompt_init,wte = self.word_embedding)
        ...
        self.in_layer = Encoder_PCA(self.patch_size * 2, self.patch_size * (2 ** configs.wavelet_j), self.word_embedding, hidden_dim=configs.d_model)
        
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            self.out_layer = nn.Linear(configs.d_model * self.patch_num * 2, self.pred_len_add)
            self.out_layer_time = nn.Linear(configs.d_model * (self.patch_num + self.prompt_length), self.pred_len_add)
    ...
    def forecast(self, x):
        ...
        x /= stdev
        next_multiple_of_four = ((L + 3) // 4) * 4
        padded_data = torch.zeros_like(x).resize_(B, next_multiple_of_four, M)
        padded_data[:, :L, :] = x
        #计算小波系数
        in_dwt = padded_data.permute(0,2,1)
...
# 获取patch块
        ...
        outputs_time = outputs_time[:, -self.pred_len_add:, :]
        outputs_time = outputs_time[:, :self.pred_len, :]
        outputs_text = outputs_text[:, -self.pred_len_add:, :]
