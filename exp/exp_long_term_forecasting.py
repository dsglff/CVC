...
def benchmark_model(model, seq_len, enc_in, device='cuda', warmup=10, runs=100):
    """
    1) 计算单次前向的总 FLOPs（GFLOPs）和可训练参数。
    2) 测量推理吞吐量：平均 samples/sec。

    Parameters:
    - model: 你的 Model 实例，需包含 .forecast(x) 方法。
    - seq_len: 输入序列长度。
    - enc_in: 特征维度。
    - device: 'cuda' 或 'cpu'。
    - warmup: 测速前的预热次数。
    - runs: 正式计时的前向次数。
    """
    dummy = torch.randn(1, seq_len, enc_in, device=device)
    ...
    print(f">> Inference throughput: {throughput:.1f} samples/sec (batch size = 1)")
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        # In your test():
        param_count = count_trainable_params(self.model)
        print(f">> Trainable parameters: {param_count:,}")
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(tqdm(train_loader)):
                ...
                # loss, task_loss, output_loss, feature_loss, a = criterion(outputs_dict, batch_y)

                train_loss.append(loss.item())

                if (i + 1) % 100 == 0:
                    # print("\titers: {0}, epoch: {1} | toloss: {2:.7f} | taskloss: {3:.7f} | feloss: {4:.7f} | outloss: {5:.7f} | aloss: {6:.7f}".format(i + 1, epoch + 1, loss.item(), task_loss.item(), feature_loss.item(), output_loss.item(), a.item()))
                    speed = iter_count * self.args.batch_size/ (time.time() - time_now) 
                    left_time = speed * ((self.args.train_epochs - epoch) * train_steps - i)
                    print('\tspeed: {:.4f}iter/s; left time: {:.4f}s'.format(speed, left_time))
                    iter_count = 0
                    time_now = time.time()
    ...
    def test(self, setting, test=0):
        ...
        if test:
            ...
            self.model.load_state_dict(torch.load(os.path.join('./checkpoints/' + setting, 'checkpoint.pth'),map_location=torch.device(self.device)))
        ...
        try:
            benchmark_model(self.model, self.args.seq_len, self.args.enc_in, device = self.device, warmup=10, runs=100)
